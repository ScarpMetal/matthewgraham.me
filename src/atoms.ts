...
import { listsShareItem } from '~/utils/functions';
...
export const displayExperiencesAtom = atom((get) => {
  ...
  return experiences.filter((experience) => listsShareItem(experience.tagIds, selectedTagIds));
});
export const projectsAtom = atom<ProjectType[]>(projectsData.data);
export const displayProjectsAtom = atom((get) => {
  const projects = get(projectsAtom);
  const selectedTagIds = get(selectedTagIdsAtom);
  return projects.filter((project) => listsShareItem(project.tagIds, selectedTagIds));
});

export const tagsAtom = atom<TagType[]>(tagsData.data);
export const selectedTagIdsAtom = atom<TagId[]>(tagsData.data.map((tag) => tag.id));
